...
\begin{proof}
    ...
    Finally by step $1$:
    \begin{align*}
        \tilde{W}(\delta_\mu\varphi_t(\mu;z_1),\delta_\mu\varphi_t(\mu;z_2)) \leq d_\alpha(z_1,z_2)(1 + aT\|\mu\|) e^{CT(1 +  T \|\mu\|)\|\mu\|_{1 + \psi_2}},
    \end{align*}
    thus ending the proof.
\end{proof}
...
\begin{align*}
    W_{1,\psi_p}(\mu,\nu) = \sup_{\substack{Lip_p(f) \leq 1 \\ |f| \leq 1 + \psi_p}} \brac{f,\mu -\nu},
\end{align*}
where
\begin{align*}
    Lip_p(f) = \sup_{x,y \in \RRP} \dfrac{\left|f(x) - f(y) \right|}{d_{p\alpha}(x,y)}\ .
\end{align*}
...
\begin{lemma}
    ...
    \begin{enumerate}[label=(\roman*)]
        ...
        \item For all $x,y \in \RRP$, $\left|\partial_x G_t(x,y) \right| \leq C \alpha x^{-1-\alpha}(1 + y^{-\alpha})$.
...
\begin{proof}
    ...
    and 
    \begin{align*} 
        (1 + x^{-\alpha})\left|\partial_x \left( h_t(x)\right)^{-1}\right| \leq \alpha x^{-1-\alpha} (1 + x^{-\alpha})  \int_0^t\|\mu_s\|\dd s e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) \mu_s(\dd y) \dd s}
    \end{align*}
    Using the form of our kernel we have:
    \begin{align*}
        e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) \mu_s(\dd y) \dd s} =  e^{-\int_0^t x^{-\alpha}\| \mu_s\| + \|\mu_s\|_{\psi_1} \dd s }\leq e^{-\int_0^t \|\mu_s\| \dd s }\ .
    \end{align*}
    and therefore finally using the inequality $xe^{-x} \leq 1$ and the fact that $t\mapsto \| \mu_t\|$ is non-increasing:
    \begin{align*}
    ...
    \end{align*}
    The function $y \mapsto K_\alpha(x,y)$ is Lipschitz and is below $(1 + x^{-\alpha})(1 + y^{-\alpha})$ we deduce that:
    \begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        G_tf(x,y) &= K_\alpha(x,y) f(x+y) h_t(x+y)\left(h_t(x)\right)^{-1}, \\
        \tilde{G}_tf(x,y) &= K_\alpha(x,y) f(x+y) \tilde{h}_t(x+y)\left(\tilde{h}_t(x)\right)^{-1}\ .
    \end{align*}
    Let $f$ be such that $Lip_p(f) \leq 1$ and $|f| \leq 1 + \psi_p$ we have for all $t \geq 0$:
    \begin{align*}
        \brac{f,h_t\mu_t - \tilde{h}_t \tilde{\mu}_t} = \brac{f,\mu - \tilde{\mu}} + \int_0^t \brac{G_sf,\mu_s \otimes h_s\mu_s} - \brac{\tilde{G}_sf,\tilde{\mu}_s\otimes\tilde{h}_s\tilde{\mu}_s} \dd s\ .
    \end{align*}
    Breaking down the term to bound in three subparts,
    \begin{subequations}
        \begin{align}
            \brac{G_tf,\mu_t \otimes h_t\mu_t} - \brac{\tilde{G}_tf,\tilde{\mu}_t\otimes \tilde{h}_t\tilde{\mu}_t} =& \brac{G_tf,\mu_t \otimes\left(h_t\mu_t -  \tilde{h}_t \tilde{\mu}_t\right)}\label{proof:eq:h_tmu_t-1} \\
            &+ \brac{G_tf - \tilde{G}_tf,\mu_t \otimes \tilde{h}_t \tilde{\mu}_t}\label{proof:eq:h_tmu_t-2}\\
            &+ \brac{\tilde{G}_tf,\left(\mu_t-\tilde{\mu}_t\right) \otimes \tilde{h}_t \tilde{\mu}_t}\label{proof:eq:h_tmu_t-3}
        \end{align}
        \label{proof:eq:h_tmu_t}
    \end{subequations}
    For \eqref{proof:eq:h_tmu_t-1} 
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    We already know from the proof of Proposition \ref{prop:wass-derivaitve} that $A_tf$ is Lpischitz with constant below $\left(1 + T\|\mu\|\right)\|\mu\|_{1 + \psi_2}$ and therefore:
    \begin{align*}
        \brac{G_tf,h_t\mu_t \otimes \left(h_tm_t - \tilde{h}_t\tilde{m}_t \right) } &\leq \left(1 + T\|\mu\|\right)\|\mu\|_{1 + \psi_2} W_{1,\psi_1}\left(h_tm_t -\tilde{h}_t\tilde{m}_t \right)
    \end{align*}
