...
We start by giving a lemma controlling the derivaitve of the $\SCE$,
\begin{lemma}
    Let $z_1,z_2 \in \RRP$, the following assertions hold:
    \begin{itemize}
        \item Assume that $\mu \in \MC_{\psi_3}(\RRP)$ for all $f \in Lip_1(d_\alpha)$:
            \begin{align*}
                \brac{f,\delta_\mu \varphi_t(\mu;z_1) - \delta_\mu \varphi_t(\mu;z_2)} \leq C_1(T,\mu + \tilde{\mu}) d_\alpha(z_1,z_2) \ .
            \end{align*}
        \item For $p \geq 1$ let $p' = \max(p+1,3)$, assume that $\mu \in \MC_{\psi_{p'}}(\RRP)$ for all $f \in Lip_1(d_{p\alpha})$:
            \begin{align*}
                \brac{f,\delta_\mu \varphi_t(\mu;z_1) - \delta_\mu \varphi_t(\mu;z_2)} \leq d_{p\alpha}(z_1,z_2) + C_2(T,\mu + \tilde{\mu}) d_\alpha(z_1,z_2)
            \end{align*}
    \end{itemize}
\end{lemma}
\begin{proof}
