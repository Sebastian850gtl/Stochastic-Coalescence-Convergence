...
\]
where $X_{i,t}$ denotes the mass of cluster $i$ at time $t$. It is well known that, under suitable conditions, a rescaled version of $M_t$ converges as $N \to \infty$ to a solution $\mu_t$ of the Smoluchowski Coagulation Equation (SCE), first introduced for modeling aerosols in 1927~\cite{smoluchowski1927drei}:
\begin{equation}\label{eq:SCE}
    ...
    \end{aligned}
\end{equation}
for bounded measurable test functions $f$. The corresponding rescaled process is denoted by $\Proc{M^N_t} := \Proc{N^{-1}M_{N^{-1}t}}$. This process is sometimes called the Marcus-Lushnikov process in reference to its introduction \cite{marcus1968stochastic,lushnikov1978coagulation} as a stochastic approximation of the solution to the Smoluchowski Coagulation Equation.
...
\]
We denote the space of Lipschitz functions for $d_{p\alpha}$ with Lipschitz semi-norm equal to $1$ be:
\[
...
\end{corollary}
\begin{proof}
    This proof relies on the argument that $\left(\RRP,d_{p\alpha}\right)$ is the same as $\left(\RRP,|\cdot|\right)$ through the lense of the isometry $x \mapsto x^{-\frac{1}{p\alpha}}$. Let $\tilde{\mu}$ be the probability measure on $\RRP$ such that for all $f$ be bounded measurable:
    ...
    \end{align*}
    where $\Seq{X_n}$ are iid random varaibles of law $\mu$. It comes naturally that for all $f$ be bounded measurable:
    \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    Because of Proposition \ref{prop:equivalent-notation} this is exactly the distance defined in Definition \ref{def:Wasserstein-general} with $x_0 = \infty$, since the space $\left(\RRP \cup \lbrace+\infty\rbrace ,d_\alpha\right)$ is complete the result follows by Proposition \ref{prop:completness-wasserstein}.
\end{proof}
...
\begin{definition}
    ...
    And we deonte the space of differentiable functions with a flat derivative $C^1(\MC_{\psi_p}^+(\RRP))$.
\end{definition}
...
The Smoluchowski Coagulation Equation ($\SCE$) of kernel $K_\alpha$ and intial value $\mu$, is an equation defined on measured value functions defined in its weak form as for all bounded measurable function $f$:
...
\begin{lemma}\label{lem:well_posedness_smol_eq}
    Let $p \geq 2$, and $\mu \in \MC_{\psi_p}^+ (\RRP)$ for all $T \geq 0$ there exists a unique solution to the Smoluchowski Coagulation Equation \eqref{eq:SCE} of parameter $K_\alpha$, said solution is a continuous map from $[0,T]$ to $\MC_{\psi_p}^+ (\RRP)$.
\end{lemma}
...
\end{proposition}
\begin{proof}
    Let $t,s$ be two time stamps, we have:
    \begin{align*}
        F(\mu_t) - F(\mu_s) = \int_0^1 \int_{\RRP} \delta_\mu F(\left[\mu_t,\mu_s \right]^\theta;z) \left(\mu_t - \mu_s \right)(\dd z) \dd \theta.
    \end{align*}
    By hypothesis on $F$ the functional part of its derivaitve $\delta_\mu F(\mu,\cdot)$ is dominated by $C(\|\mu\|_{\psi_p})(1 + \psi_p)$ and $C$ is a non-decreasing function. It comes that for all $s \geq 0$:
    \begin{align*}
...
First, we denote $D\left([0,T]; \RRP \right)$ the sapce of cadlàg functions from $[0,T]$ to $\RRP$, we equip it with the $J_1$-Skorohod topology. Let us denote by $\MC_p^+(\RRP)$ the set of finite point measures on $\RRP$ namely:
\begin{align*}
    \MC_p^+(\RRP) = \left\lbrace \sum\limits_{i = 1}^N \delta_{x_i}\  \middle|\  N\in \NN,\  (x_1,\cdots,x_N) \in \RRP \right\rbrace. 
\end{align*}
\begin{definition}
    Let $\NC$ be a random Poisson measure on $\RR^+ \times \RR^+ \times \NN^2$ with intensity $\dd s \times \dd z \times d(i,j)$ where $d(i,j) = \sum\limits_{i < j} \delta_{i,j}$. Let $K$ be a symetric kernel, $\mu^N \in \MC_p^+(\RRP)$ be a finite sum of Dirac masses. We call stochastic coalescence (\SC) process the solution of the following $SDE$:
    \begin{multline*}
...
\end{definition}
We briefly show that the jumping rate of this process is integrable and therefore almost surely finite, this suffice to show that the considered $SDE$ is well-posed indeed see \cite{applebaum2009levy}. The kernels considered $K_\alpha$ is finite on $\RRP$. Furthermore the masses in the stochastic coalescence only increase let us call $x_0$ the smallest mass in the initial data $\mu^N$. For all $t \geq 0$ and $i,j$ we have $K_\alpha(X_{i}(s-),X_{j}(s-)) \leq 2x_0^{-\alpha}$. Finally $N_t$ is non-increasing it comes that for all bounded $f$:
\begin{multline*}
...
\begin{theorem}\label{thm:difdif}
    ...
    \end{align*}
\end{theorem}
The Theorem \ref{thm:main-result} is simply theorems \ref{thm:samesame} and \ref{thm:difdif} put together. The proof of the Theorem \ref{thm:main-result-2} giving the bound of $1/N$ relies on Fournier-Guillin rate of convergence for empirical measures given in Corollary \ref{cor:fournier_guillin}.
...
\end{proposition}
\begin{proof}
    Let $\Seq{\mu_n}$ be a Cauchy sequence of measures. By the trigular inequality, for all $p,q$:
    \begin{align*}
...
\end{lemma}
Before proving this lemma we give a small thecnical proposition for differentiating a family of functions. This proposition originates from Norris in \cite{norrisSmoluchowskisCoagulationEquation1999} and his idea to use it to show that the solutions are positive.
\begin{proposition}
...
\begin{proof}[Proof of Lemma \ref{lem:fixed-point-1}]
    ...
    \right.,
    \end{align*}
    defined for all $f$ be bounded measurable, for all $t \geq 0$ by:
    \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    That being true for all $\varepsilon$ we have that, $\Proc{\mu_t}$ is a solution to the $\SCE$. Uniqueness comes from the lipschitz character of the solution given in Lemma \ref{lem:Lip-smol-TV}. Finally we show that it has a moment of order $p$. By Lemma \ref{lem:sub-add-bounf-smol} we have:
    \begin{align*}
...
\end{lemma}
\begin{remark}
    The result we will show holds also for $m \in \MC^+_{\psi_p}\left(\RRP\right) $ however the solution of the latter equation does not stay positive. Therefore we present it in all generality for a initial signed measure.
\end{remark}
\begin{remark}
...
\end{align*}
Our proof similarly as for the $\SCE$ previously is based on a fixed point and uses this subset. Corollary \ref{cor:well-posedness-derivative-unbounded-space} gives the result for all of $\MC_{\psi_p}\left(\RRP\right)$.
...
\begin{proof}[Proof of Lemma \ref{lem:well_posedness_derivative_eq}]
    ...
    \end{align*}
    We deduce that the sequence $\Proc{m^{\varepsilon_n}}$ is Cauchy for the latter distance therefore it converges in $C\left([0,T];\MC_{\psi_{p-1}}(\RRP)\right)$, let us denote $\Proc{m_t}$ the limit. Let us show that it solves Equation \eqref{eq:derivative-initial-general}. Let $\varepsilon > 0$, for all $f$ be bounded measurable:
    \begin{multline*}
...
\begin{proof}[Proof of Lemma \ref{lem:derivative-existence-and-eq}]
    ...
    \end{align*}
    \textbf{Step $1$}: Let $\mu,\nu \in \MC_{\psi_{p}}(\RRP)$, we start by showing that for all $f$ be bounded measurable:
    \begin{align*}
        (0,1) &\to \RR \\
...
% %     We have for all $f$ be bounded measurable:
% %     \begin{align*}
...
% \end{proposition}
% \begin{proof}
%     The function $\psi^c$ is sub-multiplicative therefore by Lemma \ref{lem:sub-add-bounf-smol} for all $t \geq 0$:
%     \begin{align*}
...
%     Let $\varepsilon > 0$ we have for all $f$ be bounded measurable:
%     \begin{align*}
...
%     \end{align*}
%     Notice that $G_t^\varepsilon(x,y) \leq 1$. Let for $n\in \NN$, $\varepsilon_n = \frac1n$, let $p,q$ in $\NN$ we have for all $f$ be bounded measurable:
%     \begin{multline*}
...
\begin{example}\label{exp:wasserstein-is-tricky}
    ...
    The reason why it works without too much difficulty for $K'_\alpha$ isbecause for all $x,y$, $K'_\alpha(x,y) \leq x^{-\alpha} \wedge y^{-\alpha}$. This is not true for $K_\alpha$. This kernel is of the type treated in \cite{cepedaSmoluchowskisEquationRate2011a,fournierStochasticCoalescenceHomogeneouslike2009}. 
...
\end{align*}
We know that equation verified by $h_t\mu_t$ is formally for all $f$ be bounded measurable:
\begin{align*}
...
\begin{proof}[Proof of Lemma \ref{lem:exponential-moments}]
    ...
    Let $\varepsilon > 0$ we let us denote by $\Proc{\nu_t^\varepsilon} := \Proc{h_t^\varepsilon\mu_t^\varepsilon}$, it is clear that those measures are positive. Fix $T > 0$ and consider $t \in [0,T]$, for all $f$ be bounded measurable:
    \begin{align*}
    ...
    For $n\in \NN$, define $\varepsilon_n = \frac1n$, let $p,q$ in $\NN$ we have for all $f$ be bounded measurable:
    \begin{subequations}
    \begin{align}
        \brac{f,\nu_t^{\varepsilon_p}-\nu_t^{\varepsilon_q}} =& \brac{f,\mu^{\varepsilon_p} - \mu^{\varepsilon_q}} 
        \nonumber \\
    ...
    \end{align*}
    At last we put \eqref{proof:eq:exponential-completeness} in Gronwall's shape for all $f$ be bounded measurable, such that  $|f| \leq \psi$ we have:
    \begin{multline*}
...
\begin{proof}
    ...
    By a slight abuse of notation we denote $W_{\psi}(m_t) = W_{\psi}(m_t,0)$. We have for for all $t\geq 0$, $f$ be bounded measurable:
    \begin{multline*}
    ...
    \end{align*}
    By point $(iv)$ and $(v)$ of Lemma \ref{lem:technical_G_h} we have:
    \begin{align*}
...
\begin{proof}
    ...
    \end{align*}
    By point $(vi)$ of Lemma \ref{lem:technical_G_h}:
    \begin{align*}
...
\begin{proof}
    ...
    \begin{align*}
        \brac{f,\delta_\mu\varphi_t(\mu;z)} = f(z) + \int_0^t \brac{K_\alpha f, \varphi_s(\mu)\otimes\delta_\mu\varphi_s(\mu;z)} \dd s.
    \end{align*}
    We rely again on the linearity of the derivatives equation and denote by $\Proc{m_t}$ the unique solution of equation defined for all $f$ be bounded measurable by:
    \begin{align*}
        \brac{f,m_t} = \brac{f,m_0} + \int_0^t \brac{K_\alpha f, \mu_s \otimes m_s} \dd s.
    \end{align*}
...
\begin{proof}
    ...
    Define $\Proc{m_t}$ as the unique solution of the equation defined for all $f$ be bounded measurable by:
    \begin{align*}
        \brac{f,m_t} = \brac{f,m_0} + \int_0^t \brac{K_\alpha f,\mu_s\otimes m_s} \dd s.
    \end{align*}
    ...
    \end{align*}
    For \eqref{eq:proof:wass-finale-step-1-2}, study the function $g:= f(h_t)^{-1} - f(\tilde{h}_t)^{-1}$. By point $(ii)$ of Lemma \ref{lem:technical_G_h} and Lipschitz regularity the solutions to the $\SCE$, Proposition \ref{prop:wass-sce}:
    \begin{align*}
    ...
    The equation for $\Proc{h_tm_t}$ is for all $f$ be bounded measurable:
    \begin{align*}
        \brac{f,h_t m_t} 
