...
We recall that the kernel we study is $K_\alpha$ for $\alpha >0$ and for all $x,y \in \RRP$:
\begin{align*}
    K_\alpha(x,y) = x^{-\alpha} + y^{-\alpha}.
\end{align*}
...
For an empirical measure $\mu^N$ let \(\Proc{\Phi_t(\mu^N)}\) denote the rescaled stochastic coalescence started from \(\mu^N\) with kernel \(K_\alpha\), and let \(\Proc{\varphi_t(\mu)}\) be the unique solution to the Smoluchowski coagulation equation started from \(\mu\) and kernel \(K_\alpha\).
\begin{theorem}
    Let \(\mu \in \MC^+_{\psi_3}(\RRP)\) be a probability measure, and let \(\mu^N\) be the empirical measure of \(N\) i.i.d. random variables with law \(\mu\):
    \[
    ...
    \[
    \left| \mathbb{E}\left[F\left(\Phi_t\left(\mu^N\right)\right)\right] - F\left(\varphi_t(\mu)\right) \right| \leq \left\llbracket \delta_\mu F \right\rrbracket_\mathcal{A} \cdot \frac{C\left(T, \|\mu\|, \|\mu\|_{\psi_1}, \|\mu\|_{\psi_2}\right)}{N}.
    \]
\end{theorem}
