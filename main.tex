...
\begin{proof}[Proof of Lemma \ref{lem:exponential-moments}]
    ...
    \end{align*}
    that vanishes thanks to our previous foundings. For the second we have:
    \begin{align*}
        \left\|(h_t)^{-1}\nu^\varepsilon_t - (h^\varepsilon_t)^{-1}\nu^\varepsilon_t\right\|_{\psi_1} =
    ...
    \end{align*}
    The final term thanks to \ref{lem:Lip-smol-TV}. Thus concluding this proof.
\end{proof}
...
\begin{proof}
    ...
    \begin{multline*}
        ...
        \leq K_\alpha(x,y)(x^{-\alpha} + y^{-\alpha}) \int_0^t W_{\psi_1}(\mu_s,\tilde{\mu}_s) \dd s\\
...
\end{lemma}
\begin{proof}
    We denote by $\Proc{\mu_t} = \Proc{\varphi_t(\mu)}$. For now we consider $p \geq 1$ and $\mu \in \MC_{\psi_{p'}}(\RRP)$, since the equation for the derivative of the $\SCE$  solution is linear we can make our analysis on $\Proc{m_t}$ unique solution of:
    \begin{align*}
        \brac{f,m_t} = \brac{f,m_0} + \int_0^t \brac{K_\alpha f,\mu_s\otimes m_s} \dd s.
    \end{align*}
    Taking $m_0 = \delta_z$ will give us $\Proc{h_t\delta_\mu \varphi_t(\mu;z)}$ and taking $m_0 = \delta_{z_1} - \delta_{z_2}$ will give us $\Proc{\delta_\mu \varphi_t(\mu;z_1) - \delta_\mu \varphi_t(\mu;z_2)}$. So far we treat all four cases simultaneously.
    ...
    \end{align*}    
    Notice it should be $H_tf(y,x)$ instead, however $H_tf$ is symmetric so this hardly matters. This remark will be important when handling \label{proof:eq:wass-rescaled-SCE-2} as $I_tf$ is not symetric. 
...
\begin{align*}
    Lip(f) = \sup\left\lbrace \dfrac{|f(x) - f(y)|}{d_\alpha(x,y)} , (x ,y) \in \left(\RRP\right)^2,x\neq y \right\rbrace 
\end{align*}
Recall the definition of our distance for $\mu,\nu \in \MC_{\psi_1}^+(\RRP)$:
\begin{align*}
...
\begin{proof}
    ...
    \begin{subequations}
    \begin{align}
        \brac{f, h_tm_t - \tilde{h}_t\tilde{m}_t} =& \brac{H_tf- I_tf ,h_t\mu_t \otimes \left(h_tm_t - \tilde{h}_t\tilde{m}_t \right) }\label{proof:eq:wass-finale-step-2-1}\\ 
        &+ \brac{H_tf- I_tf,\left(h_t\mu_t - \tilde{h}_t\tilde{\mu}_t\right)\otimes \tilde{h}_t\tilde{m}_t}\label{proof:eq:wass-finale-step-2-2} \\ 
        &+ \brac{H_tf-\tilde{H}_tf + \tilde{I}_tf - I_tf,\tilde{h}_t\tilde{\mu}_t\otimes \tilde{h}_t\tilde{m}_t}\label{proof:eq:wass-finale-step-2-3} 
    \end{align}
    ...
    \begin{align*}
        \brac{H_tf- I_tf ,h_t\mu_t \otimes \left(h_tm_t - \tilde{h}_t\tilde{m}_t \right) } \leq (1 + \|\mu\|T)\|\mu\|_{\psi_2}W_{\psi_1}\left(h_tm_t , \tilde{h}_t\tilde{m}_t \right).
    \end{align*}
    ...
    \begin{align*}
        \left|A_tf(y)\right| &\leq \int_0^t W_{\psi_1}(\mu_s,\tilde{\mu}_s) \dd s \times \left(1 + \psi_2(y) \right)\int_{\RRP} \left(1 + \psi_2(x) \right) \tilde{h}_t\tilde{\mu}_t(\dd x) \\
