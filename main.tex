...
% \newpage
\newpage
\section{Wasserstein regularity of solutions and derivaitve}
In this section we prove the Wasserstein regularity of the solutions to the Smolochowski coagulation equation equation. This section is very technical because of the peculiar shape of our kernel we cannot by direct means find a Wasserstein bound. An illustration of this is provided in Example \ref{exp:wasserstein-is-tricky}. This section is organized as follows, we give first the main results with little to no proofs in \ref{section:wass-main-results}, then we give all the proofs. The latter rely on some heavy technical lemmas that are all carried out in \ref{wass-technical}


Let us briefly remind the definition the Wasserstein distance we consider, for a proper introduction and defintions see Section \ref{section:Wasserstein}.

For $\mu,\tilde{\mu} \in \MC_{\psi_1}^+(\RRP)$ the Wasserstein distance associated with the metric $d_\alpha$ is defined as:
\begin{align*}
    W_{\psi_1}(\mu,\tilde{\mu}) = \sup\limits_{\substack {f \in Lip_1(d_\alpha)\\ |f| \leq 1 + \psi_1}} \brac{f,\mu - \tilde{\mu}} \ .
\end{align*}
Recall the definition of the space of Lipschitz functions from $(\RRP,d) \to (\RR,|\cdot|)$ with constant $c$ :
\begin{align*}
    Lip_c(d) = \left\lbrace f: \RRP \to \RR \middle|\quad  \sup\limits_{x \neq y \in (\RRP)^2} \dfrac{\left| f(x) - f(y)\right|}{d(x,y)}\leq  c \right\rbrace \ .
\end{align*}

One of the key technical insights underlying our analysis is that solutions to the SCE gain exponential moments instantaneously—even if the initial data lacks them. This regularization phenomenon plays a central role in controlling the Wasserstein distance and in establishing the fine properties of the dynamics.

\subsection{Main results}\label{section:wass-main-results}
In this section we give the main results of this section that will be of use in the main results of this work. We give no proof with the exeption of one that is simple enough. The proofs are listed 
Recall that for $\mu \in \MC^+_{\psi_2}(\RRP)$, $\Proc{\varphi_t(\mu)}$ denotes the solutions to the Smolochowski coagulation equation started from $\mu$ with kernel $K_\alpha$, and for $\mu \in \MC^+_{\psi_3}(\RRP)$, $\Proc{\delta_\mu\varphi_t(\mu)}$ its derivatve. 

\subsection{Proof srategies}

Now to illustrate why the remainder of the section is so technically involved.
...
\end{align*}
We know that equation verified by $h_t\mu_t$ is for all $f \in C_b(\RRP)$:
\begin{align*}
    \brac{f,h_t\mu_t} = \brac{f,\mu} + \dfrac{1}{2}\int_{\RRP} \int_{\RRP} K_\alpha(x,y) fh_t(x+y) \mu_t(\dd x) \mu_t (\dd y) \ .
\end{align*}
As we will see this formulation gets rid of the difficulty we previously had however computations are tedious. 
...
\begin{proposition}\label{prop:wass-sce}
    ...
    \[
        C(T,m) = (1 + T\|\mu\|)e^{CT\left(\|m\|_{\psi_1}  +  T\|m\|\|m\|_{\psi_2}\right)}\ .
    \]
...
\begin{proof}
    ...
    \begin{align*}
        W_{\psi_1}^L(\varphi_t(\mu),\varphi_t(\tilde{\mu})) \leq C(T,\mu + \tilde{\mu})W_{\psi_1}^L(\mu,\tilde{\mu})\ .
    \end{align*}
    ...
    \begin{align*}
        ...
        \leq& \brac{1,\mu-\nu} - \dfrac12\int_0^t \brac{1,\varphi_s(\mu)-\varphi_s(\tilde{\mu})}\int_{\RRP} x^{-\alpha} \left(\varphi_s(\mu) + \varphi_s(\tilde{\mu})\right)(\dd x) \dd s\\
        &+ \dfrac12\int_0^t \brac{1,\varphi_s(\mu)+\varphi_s(\tilde{\mu})} W_{\psi_1}^L\left(\varphi_s(\mu),\varphi_s(\tilde{\mu})\right) \dd s
    \end{align*}
    and therefore by Gronwall lemma,
    \begin{multline*}
        \brac{1,\varphi_t(\mu)-\varphi_t(\tilde{\mu})} \leq \left(\brac{1,\mu-\nu} + \dfrac12\int_0^t \brac{1,\varphi_s(\mu)+\varphi_s(\tilde{\mu})} W_{\psi_1}^L\left(\varphi_s(\mu),\varphi_s(\tilde{\mu})\right) \dd s\right) \\
        \times e^{-\frac12\int_0^t \int_{\RRP} x^{-\alpha} \left(\varphi_s(\mu) + \varphi_s(\tilde{\mu})\right)(\dd x) \dd s}\\
        \leq \left| \brac{1,\mu-\nu}\right| + T\brac{1,\mu + \tilde{\mu}} C(T,\mu + \tilde{\mu})W_{\psi_1}^L\left(\mu,\tilde{\mu}\right)\ .
    \end{multline*}
    ...
    \begin{align*}
        W_{\psi_1}(\varphi_t(\mu),\varphi_t(\tilde{\mu}))\leq \left( 1 + T\brac{1,\mu + \tilde{\mu}} \right) C(T,\mu + \tilde{\mu})W_{\psi_1}\left( \mu,\tilde{\mu}\right)\ .
    \end{align*}
\end{proof}
...
We give a lemma controlling the derivaitve of the $\SCE$,
\begin{lemma}\label{lem:wass-rescaled-derivaitve}
...
\begin{proof}
    ...
    \end{align*}
    If $p = 1$ then a Gronwall lemma gives us the first result. Then for $p > 1$ we can use the $\psi_1$ bound and proceed by brute force.
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    We already saw in the proof of Lemma \ref{lem:wass-rescaled-derivaitve} that the latter in $Lip_{C\|\mu\|_{\psi_{p+1}}}(d_{\alpha})$ for all $f$ in $Lip_1(d_{p\alpha})$ and consequently term \eqref{proof:eq:h_tmu_t-1} gets the following bound:
    \begin{align*}
        \brac{H_tf,\mu_t \otimes\left(h_t\mu_t -  \tilde{h}_t \tilde{\mu}_t\right)} &= \brac{A_tf,h_t\mu_t -  \tilde{h}_t \tilde{\mu}_t}\\
...
\begin{proof}
    ...
    The rest of this step is devoted to bounding the remaining term. We proceed similarly as in the first step by showing that $A_tf - \tilde{A}_tf$ is Lpischitz with a constant controlled by $W_{\psi_1}(\mu,\tilde{\mu})$ and use Proposition \ref{prop:wass-derivaitve} to conclude. We differentiate the latter function:
    \begin{align*}
