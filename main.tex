...
\begin{theorem}\label{thm:samesame}
    ...
    \end{align*}
\end{theorem}
Our proof is based on the following relation, we took inspiration from \cite{kolokoltsov2010central} that uses it in his proof of a central limit theorem.
...
\begin{proof}
    ...
    \begin{multline*}
        \brac{K_\alpha \psi_p\sigma_s, \mu^\varepsilon_s\otimes m^\varepsilon_s}\\
        = \int_{\RRP\times\RRP} K_\alpha(x,y) \left[\psi_p(x+y)\sigma_s(x+y) - \psi_p(x)\sigma_s(x) - \psi_p(y)\sigma_s(y) \right]\mu^\varepsilon_s(\dd x)\mu_s(\dd y) \\
    \end{multline*}
...
\begin{align*}
    \left| \left(\LC^N - \LC\right)F\circ \varphi_s (\mu) \right| \leq \frac{9}{2N}\|\delta_\mu^2 F\|_{\infty}\langle K,\mu\otimes\mu\rangle + \frac{3}{N}\|\delta_\mu F\|_{\infty}\int_{\RR^+} K_{\alpha}(x,x)\mu(\dd x)
\end{align*}
\end{lemma}
...
\begin{proof}
    ...
    \end{align*}
    By hypothesis on $F$, differentiability of $\mu \mapsto \varphi_t(\mu)$, and Lipschitz regualrity of its derivaitve we can apply the chain rule \red{ref prop}. Let $\nu,z \in \MC^+_{\psi_1}(\RRP),\RRP$ we have:
    \begin{align*}
    ...
    For \eqref{proof:eq:samesame-part1} by hypothesis on $F$ we have the function:
    \begin{align*}
    ...
    \begin{align*}
        ...
        &\leq C(\psi_1(x) + \psi_1(y))
    \end{align*}
    And we get:
    \begin{align*}
        \brac{\delta_\mu F\left(\varphi_s\left(\mu^\theta_N\right),\cdot\right) - \delta_\mu F(\varphi_s(\mu);\cdot),\delta_\mu \varphi_s (\mu;z)} \leq \dfrac{C(T,\mu)}{N}(1 + \psi_1(z))
    \end{align*}
...
\begin{align*}
    ...
    &+ \frac{3}{N}\|\delta_\mu F\|_{\infty}\int_0^t\E{\int_{\RR^+} K_{\alpha}(x,x)\varphi(s,\Phi(t-s,\mu^N))(\dd x)}.
\end{align*}
Let us handle the second term:
\begin{align*}
    \E{\int_{\RR^+} K_{\alpha}(x,x)\varphi(s,\Phi(t-s,\mu^N))(\dd x)} 
