...
In this proof we use the characterization of Lipschitz functions for $d_\alpha$ given by Proposition \ref{prop:characterisation-lip-1}, namely a function $f$ is Lipschitz with $Lip(f) \leq 1$ if and only if it is almost everywhere differentiable and its derivative enjoys the following bound: for almost every $x$, $|f'(x)| \leq \alpha x^{-1-\alpha}$.
\begin{proof}
    We adopt the following notations, let $a \in \RR^+$ we denote by $\tilde{W}$ the value of the supremum:
    \begin{align*}
        \tilde{W}(\mu,\nu) = \sup\limits_{\substack{Lip(f) \leq 1, \\|f(x)| \leq a + x^{-\alpha}}} \brac{f,\mu-\nu},
    \end{align*}
    the first presented case corresponding to $a = 0$ and the second $a = 1$. We denote $\Proc{\varphi_t(\mu)}$ by $\Proc{\mu_t}$ and by $\gamma_t = \int_0^t \brac{1,\mu_s} \dd s$ a quantity that will appear numerous times in the proof.
...
\begin{proposition}
    ...
    \[
        C(T,\mu,\tilde{\mu}) = \left( 1 + T(\|\mu\|+ \|\tilde{\mu}\|)\right)e^{CT(\|\mu\|_{\psi_1} + \| \tilde{\mu}\|_{\psi_1} +  T(\|\mu\|+ \|\tilde{\mu}\|)(\|\mu \|_{\psi_2} + \|\tilde{\mu}\|_{\psi_2}))}
    \]
\end{proposition}
\begin{proof}
    ...
    \end{multline*}
    The function $f+a$ verifies $Lip(f+a) \leq 1$ and $|f(x) + a| \leq 1 + x^{-\alpha}$. Therefore from Proposition \ref{prop:lip-regularity-derivaitve} the function $z \mapsto \brac{f+a, \delta_\mu\varphi_t(\theta\mu + (1-\theta)\tilde{\mu};z)}$ is Lipschitz with $Lip$ pseudo norm under:
    \begin{align*}
...
We want now to prove the main result of this part 
