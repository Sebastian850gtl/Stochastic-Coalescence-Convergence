...
\begin{proof}
    ...
    \begin{align*}
        ...
        &\leq C\gamma_t \int_{\RRP}(1 + y^{-2\alpha}) e^{y^{-\alpha}\gamma_t} \mu_t(\dd y)\\
        &\leq C\gamma_t \|\mu\|_{1 + \psi_2}\ .
    \end{align*}
    ...
    \end{multline*}
    In conclusion we found that the previous function $A_tf$ is Lipschitz with a Lip pseudo norm smaller than $C(\gamma_t \|\mu\|_{1 + \psi_2} + \|\mu\|_{1 + \psi_1})$ from our original equation \eqref{eq:proof:wass-derivaitve} we have:
    \begin{multline*}
        \brac{fh_t,\delta_\mu\varphi_t(\mu;z_1) -\delta_\mu\varphi_t(\mu;z_2)} \\
        = f(z_1) - f(z_2) + \int_0^t \brac{A_sf,h_s\delta_\mu\varphi_s(\mu;z_1) - h_s\delta_\mu\varphi_s(\mu;z_2)} \dd s \\
        \leq d_\alpha(z_1,z_2) + C\int_0^t (\|\mu\|_{1 +\psi_1} + \gamma_s \|\mu\|_{1 + \psi_2}) \tilde{W}(h_s\delta_\mu\varphi_s(\mu;z_1),h_s\delta_\mu\varphi_s(\mu;z_2)) \dd s 
    \end{multline*}
    where we remind that $\gamma_t = \int_0^t \brac{1,\mu_s} \dd s \leq t\|\mu\|$. By passing to the supremum on the LHS and a use of the Gronwall lemma one gets that for all $t \in [0,T]$:
    \begin{align*}
        \tilde{W}(h_t\delta_\mu\varphi_t(\mu;z_1),h_t\delta_\mu\varphi_t(\mu;z_2)) \leq d_\alpha(z_1,z_2) e^{CT(\|\mu\|_{1 + \psi_1} +  T\|\mu\|\|\mu\|_{1 + \psi_2})}\ .
    \end{align*}
    Finally by step $1$:
    \begin{align*}
        \tilde{W}(\delta_\mu\varphi_t(\mu;z_1),\delta_\mu\varphi_t(\mu;z_2)) \leq d_\alpha(z_1,z_2)(1 + aT\|\mu\|) e^{CT(\|\mu\|_{\psi_1} +  T\|\mu\|\|\mu\|_{\psi_2})},
    \end{align*}
    thus ending the proof.
\end{proof}
...
\begin{proposition}
    ...
    \[
        C(T,m) = \left( 1 + T\|m\|\right)e^{CT\left(\|m\|_{1 + \psi_1}  +  T\|m\|\|m\|_{1+\psi_2}\right)}
    \]
\end{proposition}
\begin{proof}
    Let $f $ be such that $Lip(f) \leq 1$ and $|f| \leq \psi_1$, we have:
    \begin{align*}
        \brac{f,\varphi_t(\mu)-\varphi_t(\tilde{\mu})} + \left| \varphi_t(\mu)(\RRP) - \varphi_t(\tilde{\mu})(\RRP)\right|
        &= \brac{f + a,\varphi_t(\mu)-\varphi_t(\tilde{\mu})},
    \end{align*}
    where $a = sign(\varphi_t(\mu)(\RRP) - \varphi_t(\tilde{\mu})(\RRP))$. By definition of the flat derivaitve we have:
    \begin{multline*}
        \brac{f,\varphi_t(\mu)-\varphi_t(\tilde{\mu})} + \left| \varphi_t(\mu)(\RRP) - \varphi_t(\tilde{\mu})(\RRP)\right| \\
        = \int_0^1 \int_{\RRP} \brac{f + a, \delta_\mu\varphi_t(\theta\mu + (1-\theta)\tilde{\mu};z)} \left(\mu-\tilde{\mu}\right)(\dd z)\ .
    \end{multline*}
    The function $f+a$ verifies $Lip(f+a) \leq 1$ and $|f(x) + a| \leq 1 + x^{-\alpha}$. Therefore from Proposition \ref{prop:lip-regularity-derivaitve} the function 
    \[z \mapsto \brac{f+a, \delta_\mu\varphi_t(\theta\mu + (1-\theta)\tilde{\mu};z)}\]
    is Lipschitz with $Lip$ pseudo norm under:
    \begin{align*}
        (1 + \|\theta\mu + (1-\theta)\tilde{\mu}\|T)e^{CT(\|\theta\mu + (1-\theta)\tilde{\mu}\|_{\psi_1} +  T\|\theta\mu + (1-\theta)\tilde{\mu}\|\|\theta\mu + (1-\theta)\tilde{\mu}\|_{\psi_2})},
    \end{align*}
    that is smaller than $C(T,\mu + \tilde{\mu})$ and thus ending the proof.
\end{proof}

We prove now the main result of this part,

\begin{proposition}
    Let $\mu,\tilde{\mu} \in \MC_{\phi_3}^+(\RRP)$, let $f$ be a Lipschitz \textbf{bounded} function such that $Lip(f) \leq 1$ and $|f| \leq 1 + \psi_n$ \red{On a $\psi_n$ !!!}. For all $t \geq 0$, the function:
    \[
...
\begin{proof}
    ...
    \end{align*}
    In the opposite case we can simply revert the role of $\mu$ and $\tilde{\mu}$ in what follows.
    \begin{align*}
    ...
    \begin{align*}
        \brac{f(h_t)^{-1} - f(\tilde{h}_t)^{-1},\tilde{h}_t\tilde{m}_t} = \brac{f(h_t)^{-1}\tilde{h}_t - f,\delta_\mu\varphi_t(\tilde{\mu}:z_1)- \delta_\mu\varphi_t(\tilde{\mu}:z_2)}\ .
    \end{align*}
    Let us study the function $f\left((h_t)^{-1}\tilde{h}_t - 1\right)$ and show that it is Lpischitz in order to use the Proposition \ref{prop:wass-derivaitve} and get the desired bound. We differentiate the latter function that we denote by $g$:
    \begin{multline*}
        g'(x) = f'(x) \left(e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd x)\dd s} - 1\right) \\
            - f(x) \int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd x)\dd s e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd x)\dd s}
    \end{multline*}
    We now use our assumptions that makes the term inside the exponentials negative. We have:
    \begin{multline*}
    ...
    \begin{multline*}
        |f(x)| \left|\int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd x)\dd s\right| \\
        \leq \alpha x^{-1-\alpha} \int_0^t \brac{1,\mu_s-\tilde{\mu}_s} \dd s \times e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd x)\dd s}\\
