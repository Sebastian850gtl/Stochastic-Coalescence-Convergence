...
\end{proof}
We also have the following characterisation.
\begin{proposition}
...
\end{proof}
\subsubsection*{Considered spaces}
We mention that we are considering measues that have finite negative $\alpha$-moment. In general we are interested in the functions $\psi_p$ defined for all $p \geq 0$ by:
\[
\psi_p(x) = x^{-p\alpha}.
\]
We primarily work with the spaces $\MC_{\psi_p}(\RRP)$ and $\MC^+_{\psi_p}(\RRP)$, the spaces of finite signed and positive measures with finite negrative $\alpha p$-moment. Note that if $p \leq q$, then:
\[
1 + \psi_p(x) \leq 2(1 + \psi_q(x)),
\]
and therefore $\MC_{\psi_q}(\RRP) \subset \MC_{\psi_p}(\RRP)$. A central tool in our analysis is a Wasserstein-type distance that metrizes the weak topology on $\MC^+_{\psi_p}(\RRP)$ and the convergence of the negative $\alpha p$-moment. First, define the distance $d_{p\alpha}$ on $\RRP$ by:
\[
...
Our final result is based on the convergence rate of sequences of empirical measures toward their law. For $\mu$ a probability measure an empirical measure of iid random variables of law $\mu$ is a measure $\mu^N$ of the form:
    \begin{align*}
        \mu^N = \dfrac1N \sum\limits_{n = 1}^N \delta_{X_n},\quad \Seq{X_n}\ \text{iid of law}\ \mu .
    \end{align*}
\begin{theorem}[Wasserstein convergence rate \cite{fournier2015rate}]\label{thm:fournier-guillin}
    Let $W_1$ be the Wasserstein-$1$ distance \cite{villani2008optimal} or Definition \ref{def:Wasserstein}. Assume that $\mu$ as a finite moment of order $q > 2$, i.e $\brac{x^q,\mu} < \infty$. There exists a constant $C>0$ such that for all $N \geq 1$:
    \begin{align*}
        W_1\left(\mu^N,\mu\right) \leq \brac{x^q,\mu}^{\frac{1}{q}} \dfrac{C}{\sqrt{N}}\  \text{a.s.}
    \end{align*}
\end{theorem}
This motivates the use of a Wasserstein-type distance on $\MC_{\psi_p}(\RRP)$ and $\MC^+_{\psi_p}(\RRP)$. Below we give a distnace on $\MC^+_{\psi_p}\left(\RRP \cup \lbrace+\infty\rbrace\right)$. Let the space Lipschitz functions for $d_{p\alpha}$ with constant equal to $1$ be defined by:
\[
...
\begin{definition}
    ...
    \]
\end{definition}
This is the Kantorovich-Rubinstein dual formulation of some Wasserstein distnace on the space $\MC^+_{\psi_p}(\RRP)$. Since two measures on that space may not have equal total mass the link is not direct and is thouroughly explained in Section~\ref{section:Wasserstein}. This distance metrizes the weak convergence plus the convergence of negative $p\alpha$-moments , this is classic \cite{villani2008optimal}. We can give a corollary to the result of \cite{fournier2015rate} on the convergence rate of empirical measures for $W_{\psi_p}$. 
\begin{corollary}\label{cor:fournier_guillin}
    Assume that $\mu$ is a probability measure in $\MC_{\psi_q}$ with $q > 2p$ then there exists a constant $C>0$ such that for all $N \geq 1$ and $\mu^N$ an empirical measure of law $\mu$:
    \begin{align*}
        W_{\psi_p}\left(\mu^N,\mu \right) \leq \brac{\psi_q,\mu}^{\frac{p}{q}}  \dfrac{C}{N}.
    \end{align*} 
\end{corollary}
\begin{proof}
    ...
    \begin{align*}
        W_{\psi_p}\left(\mu^N,\mu\right) \leq \brac{x^q,\tilde{\mu}} \dfrac{C}{\sqrt{N}}.
    \end{align*}
    Finally $\brac{x^q,\tilde{\mu}} = \brac{\psi_q,\mu}$ by definition of $\tilde{\mu}$ ending the proof.
\end{proof}
...
\end{align*}
It is a Cauchy sequence but does not converge toward any measure in $\MC^+_{\psi_p}\left(\RRP \right)$, as the limit is the measure $\delta_{\infty}$. 
\begin{proposition}
...
A solution for avoid having to add a point at infinity is to use a composite distance equal to $d_{p\alpha}$ close to $0$ on $(0,1]$ because we need it to capture the singularity of our kernel and using the absolute value on say $(1, \infty)$. 
...
\end{proof}
\begin{remark}
    The second expression can be conveniently extended to any measure with finite order $-\alpha$ moment namely $\MC_{\psi_1}^+(\RRP)$.
\end{remark}
...
In this section, we state the main assumptions and the main theorem of this work. It is a convergence rate of observables of the stochastic coalescence properly rescaled to the unique solution of the Smoluchovski coagulation equation. This theorem does not provide a proof of weak convergence because the class of functionals considered is probably too restrictive. Its interest is in application where we are interested in quantities that can be expessed in terms of observables, meaning the expected value of some functional of the process. Its second interest is that unlike pathwise bounds that give a convergence rate of $N^{-\frac{1}{2}}$ see \cite{cepedaSmoluchowskisEquationRate2011a} this gives us a $\frac{1}{N}$ which is also a clear improvement.
...
The infimum of all constants \(C > 0\) for which the above inequalities hold is denoted by \(\left\llbracket \delta_\mu F \right\rrbracket_{\A}\).
...
\begin{theorem}
    For some $\kappa > 0$, let \(\mu \in \MC^+_{\psi_{4 + \kappa}}(\RRP)\) be a probability measure, and let \(\mu^N\) be the empirical measure of \(N\) i.i.d. random variables with law \(\mu\):
    \[
        \mu^N = \frac{1}{N} \sum_{n = 1}^N \delta_{X_n}, \quad X_1, \dots, X_N \sim \mu \text{ i.i.d.}
    \]
    For any function \(F : \MC_{\psi_1}(\RRP) \to \RR\) satisfying \(\A\), and for all \(t \in [0, T]\), we have:
    \[
    \left| \mathbb{E}\left[F\left(\Phi_t\left(\mu^N\right)\right)\right] - F\left(\varphi_t(\mu)\right) \right| \leq \left\llbracket \delta_\mu F \right\rrbracket_{\A}  \frac{C\left(T, \|\mu\|, \|\mu\|_{\psi_1}, \|\mu\|_{\psi_2}\right)}{N}.
    \]
\end{theorem}

The requirement $4 + \kappa$ arises solely from the need to apply the Corollary of Fournier-Guillin's theorem \ref{cor:fournier_guillin} to $W_{\psi_2}\left(\mu^N,\mu\right)$. A distance that appears when trying to get estimates of the Wasserstein regularity of the flat derivative of the solution to the $\SCE$. It could be possible that the required moments is not optimal.
% \red{Other formulation}
% For $f : (E,d_E) \to (F,d_F)$ we denote by $\left\llbracket f \right\rrbracket_{(E,d_E),(F,d_F)}$ the Lipschitz pseudo norm:
% \begin{align*}
%     \left\llbracket f \right\rrbracket_{(E,d_E),(F,d_F)} = \sup\limits_{x \neq y \in E} \dfrac{d_F(f(x),f(y))}{d_E(x,y)}.
% \end{align*}

% \begin{itemize}
%     \item For all $\mu, z \in \MC^+_{\psi_1}(\RRP)\times \RRP$: \[\delta_\mu F (\mu,z) \leq C\left(1 + \psi_1(z)\right).\]
%     \item For all $\mu \in \MC^+_{\psi_1}(\RRP)$:\[\left\llbracket \delta_\mu F(\mu,\cdot)\right\rrbracket_{(\RRP,d_\alpha)} < \infty.\]
%     \item For all $z \in \RRP$:\[\left\llbracket \delta_\mu F(\cdot,z)\right\rrbracket_{\left(\MC_{\psi_1}(\RRP),W_{\psi_1}\right)} < \infty.\]
%     \item And finally:\[\left\llbracket \delta_\mu F \right\rrbracket_{\left(\MC^+_{\psi_1}(\RRP) \times \RRP, W_{\psi_1} \times d_\alpha\right)}  < \infty.\]
% \end{itemize}
