...
We give a formal definition as an integral on a Poisson random measure taken from \cite{fournier2006some,fournier2009stochastic}. First of we denote $D\left([0,T]: \XF\right)$ the sapce of cadlàg functions from $[0,T]$ to $\XF$ where $\XF$ is Polish. We equip it with the $J_1$-Skorohod topology. Let us denote by $\MC_p^+(\RRP)$ the set of finite point measures on $\RRP$ namely:
\begin{align*}
...
\begin{align*}
    \E{\int_0^t \int_0^\infty \int_{i <j }  \indic{z \leq K(X_{i}(s-),X_{j}(s-))} \indic{j \leq N_{s-}} \\ \times f\left(X_{i}(s-) + X_{j}(s-)\right) - f\left(X_{i}(s-)\right) - f\left(X_{j}(s-)\right)\mathcal{N}(ds,dz,d(i,j))} \leq 3\|f\|_{\infty}\dfrac{N(N-1)}{2} \times 2x_0^{-\alpha} < \inft.
\end{align*}
