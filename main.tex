...
In this work, we describe the evolution of particle systems using measure-valued stochastic processes, and the Smoluchowski coagulation equation is formulated as an equation on measures. Elements of measure theory and the relevant topological framework for spaces of measures are introduced in Section~\ref{section:analysis-measures}. If any ambiguity arises regarding the definitions or objects used below, we refer the reader to that section for clarification.

\subsection{Notations}
We use “positive” to mean $> 0$ and “non-negative” to mean $\geq 0$.
...
We denote by $\MC^+(\RRP)$ the set of finite positive measures on $\RRP$ and by $\MC(\RRP)$ the set of finite signed measures. For a measure $\mu \in \MC(\RRP)$, we denote by $|\mu| \in \MC^+(\RRP)$ its total variation measure, and by $\|\mu\|$ its total variation norm. A detailed introduction to measure spaces and the associated topologies considered in this work is given in Section~\ref{section:analysis-measures}.

\subsection{Topological considerations on spaces of measures}

We now introduce the spaces of measures used to state our assumptions and main results. Let $\psi$ be a continuous, positive function. We define:
\[
...
We define, for all $p \geq 0$,
\[
\psi_p(x) = x^{-p\alpha}.
\]
Note that if $p \leq q$, then:
\[
1 + \psi_p(x) \leq 2(1 + \psi_q(x)),
\]
so that $\MC_{\psi_q}(\RRP) \subset \MC_{\psi_p}(\RRP)$.

We primarily work with the spaces $\MC_{\psi_p}(\RRP)$ and $\MC^+_{\psi_p}(\RRP)$. Those spaces are to be interpreted as the space of measures with finite $p$ moment.
...
A central tool in our analysis is a Wasserstein-type distance that metrizes the weak topology on $\MC^+_{\psi_p}(\RRP)$. First, define the distance $d_{p\alpha}$ on $\RRP$ by:
\[
d_{p\alpha}(x,y) = |x^{-p\alpha} - y^{-p\alpha}|.
\]
Let $Lip_1(d_{p\alpha})$ denote the space of functions $f:\RRP \to \RR$ such that:
\[
\sup_{x \neq y \in \RRP} \dfrac{|f(x) - f(y)|}{d_{p\alpha}(x,y)} \leq 1.
\]

Then the Wasserstein distance on $\MC^+_{\psi_p}(\RRP)$ is defined via the dual formulation:
\[
W_{\psi_p}(\mu,\nu) = \sup_{\substack{f \in Lip_1(d_{p\alpha}) \\ |f| \leq 1 + \psi_p}} \brac{f, \mu - \nu}.
\]
Note: Since we do not assume $\mu$ and $\nu$ have the same total mass, the boundedness condition on $f$ is necessary to ensure the supremum is finite. A precise construction of this distance, along with its connection to the classical Wasserstein distance for probability measures, is provided in Section~\ref{section:Wasserstein}. The distance $W_{\psi_1}$ plays a central role, as $d_\alpha$ matches the singularity structure of the kernel $K_\alpha$ under consideration.

Regarding completeness: for any continuous and positive $\psi$, the space $\left(\MC_{\psi}(\RRP), \|\cdot\|_{\psi}\right)$ is a Banach space. Although we primarily rely on completeness in total variation, we also mention the Wasserstein setting: for all $p > 0$, the space $\MC^+_{\psi_p}(\RRP)$ is complete under the distance $W_{\psi_p}$. However, $\left(\MC_{\psi_p}(\RRP), W_{\psi_p} \right)$ is \textbf{not} complete \cite{piccoli2019wasserstein}. For a Cauchy sequence of signed measures to converge in $W_{\psi_p}$, tightness is also required.
...
\end{lemma}
We show in Lemma \ref{lem:well_posedness_derivative_eq} that upon having $\mu \in \MC_{\psi_2}^+(\RRP)$ there is a unique solution of \eqref{eq:SCE} that is a continuous map in 
\begin{align*}
    C\left(\RR^+:\MC^+_{\psi_2}(\RRP)\right).
\end{align*}
...
\begin{lemma}\label{lem:derivative-existence-and-eq}
    Let $T > 0$, for all $t \in [0,T)$ the function $ \mu \mapsto \varphi_t(\mu)$ has a linear derivative that we denote by $\delta_\mu\varphi_t$, furthermore for all $z \in \RRP, \mu \in \MC_{\phi_p}(\RRP)$, $\delta_\mu\varphi_t(\mu;z)$ is the unique solution to for all $f \in C_b(\RRP)$:
    \begin{align*}
        \brac{f,\delta_\mu\varphi_t(\mu;z)} = f(z) + \int_0^t \brac{Kf,\varphi_s(\mu)\otimes \delta_\mu\varphi_s(\mu;z)} \dd s.
    \end{align*}
\end{lemma}
\subsection{Construction of Stochastic Coalescence}
...
\end{remark}
When $F$ has a flat derivaitve the previous expression can be expressed in terms of the linear derivative of $F$
...
\end{theorem}
\begin{proof}
    The function $F\circ \varphi_t$ has a linear derivative and therefore:
    \begin{multline*}
