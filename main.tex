...
\]
where $X_{i,t}$ denotes the mass of cluster $i$ at time $t$. It is well known that, under suitable conditions, a rescaled version of $M_t$ converges as $N \to \infty$ to a solution $\mu_t$ of the Smoluchowski Coagulation Equation (SCE):
\begin{equation}\label{eq:SCE}
    ...
    \end{aligned}
\end{equation}
for bounded measurable test functions $f$. The mentioned rescaled process we denote $\Proc{M^N_t} := \Proc{N^{-1}M_{N^{-1}t}}$.
...
\end{align*}
where $K_\alpha f(x,y) = K_{\alpha}(x,y)\left(f(x+y) - f(x) - f(y) \right)$. Results on the well-posedness of this equation for this kernel are already known, see \cite{norris1999Smoluchowski} and for completeness, we recall and reprove these results in Section \ref{section:well-posedness-SCE}. In particular, we show the following:
...
\end{lemma}
Before proving this lemma we give a small thecnical proposition for differentiating a family of functions. This proposition originates from Norris in \cite{norris1999Smoluchowski} and his idea to use it to show that the solutions are positive.
\begin{proposition}
...
\begin{example}\label{exp:wasserstein-is-tricky}
    ...
    \end{align*}
    One can check with the bound stated in Lemma \ref{lem:sub-add-bounf-smol} that this is a Lipschitz function if $\mu,\tilde{\mu} \in \MC_{\psi_2}(\RRP)$. It works with $K'_\alpha$ because for all $x,y$, $K'_\alpha(x,y) \leq x^{-\alpha} \wedge y^{-\alpha}$. This is not true for $K_\alpha$.
\end{example}
