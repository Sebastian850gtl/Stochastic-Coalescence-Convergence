...
\subsection{\red{Total variation} Topology on spaces of measures}
...
This section is very technical due to the singular structure of our coagulation kernel, which prevents us from obtaining Wasserstein bounds through direct methods. An illustration of this difficulty is given in Example~\ref{exp:wasserstein-is-tricky}. This section is organized as follows, we give first the main results with little to no proofs in \ref{section:wass-main-results}. The remainder is the proofs of said main results, starting with the generation of exponential moments in Section \ref{section:wass-exponential}. The rest of the theorems rely on some heavy technical lemmas that are all carried out in \ref{wass-technical}. Finally the two remaining sections are for the proofs of Propositions \ref{prop:wass-derivative} and \ref{wass:final}

\subsection{Main results}\label{section:wass-main-results}
We now present the main results of this section, which will be instrumental in deriving the core findings of this work. Proofs are omitted except for one that is straightforward enough to be included here. Full proofs can be found in later sections.
...
\begin{proposition}[Wasserstein Comparison of Derivatives]\label{wass:final}
    Let $\mu, \tilde{\mu} \in \MC_{\psi_3}^+(\RRP)$, and fix $T > 0$. Then there exists a constant $C(T,\mu,\tilde{\mu})$ such that:
    \begin{enumerate}
        \item For all $z \in \RRP$,
        \[
        W_{\psi_1}\left( \delta_\mu \varphi_t(\mu; z), \delta_\mu \varphi_t(\tilde{\mu}; z)\right) \leq (1 + \psi_1(z)+ \psi_2(z)) C(T,\mu,\tilde{\mu}).
        \]
        \item For all $z_1,z_2 \in \RRP$,
        \begin{multline*}
...
\begin{proof}
    ...
    \end{align*}
    Given the form of the kernel, we compute:
    \begin{align*}
        \int_{\RRP} K_{\alpha}(x,y) \mu_s(\dd y) = x^{-\alpha} \brac{1,\mu_s} + \brac{\psi_1,\mu_s}.
    \end{align*}
    Dividing both sides by $e^{\int_0^t \brac{\psi_1,\mu_s} \dd s}$ gives the result.
\end{proof}
...
\begin{example}\label{exp:wasserstein-is-tricky}
    ...
    \end{align*}
    One can check with the previous bound stated in Lemma \ref{lem:bound_smol_1} that this is Lipschitz function if $\mu,\tilde{\mu} \in \MC_{\psi_2}(\RRP)$. It works with $K'_\alpha$ because for all $x,y$, $K'_\alpha(x,y) \leq x^{-\alpha} \wedge y^{-\alpha}$. This is not true for $K_\alpha$.
\end{example}
...
\begin{align*}
    h_t(x) = \int_{\RRP} K_\alpha(x,y) \mu_t(\dd y).
\end{align*}
...
\end{align*}
As we will see this formulation gets rid of the difficulty we previously had, however computations are tedious. The previous computation is formal because for now there is no guarentee that $h_t\varphi_t(\mu)$ defines a finite measure. The purpose of the next section is to show that it is in fact the case with very little assumptions on $\mu$.
...
\begin{proof}
    ...
    \end{align*}
    Given the shape of our kernel and the assumption on the variations of $\psi$ we have that:
    \begin{align*}
    ...
    \begin{align*}
        ...
        &= \brac{\psi,\mu^\varepsilon} + \int_0^t \brac{\psi_1,\mu_s} \brac{\psi h^\varepsilon_s,\mu^\varepsilon_s}\dd s.
    \end{align*}
...
\begin{proof}[Proof of Lemma \ref{lem:exponential-moments}]
    ...
    \begin{align}
        \brac{f,\nu_t^{\varepsilon_p}-\nu_t^{\varepsilon_q}} =& \brac{f,\mu^{\varepsilon_p} - \mu^{\varepsilon_q}} 
        \nonumber \\
        &+ \dfrac12\int_0^t \int_{\RRP}\int_{\RRP}K_\alpha(x,y)f(x+y) G^{\varepsilon_p}_s(x,y)(\nu_s^{\varepsilon_p} + \nu_s^{\varepsilon_q})(\dd x)(\nu_s^{\varepsilon_p} -\nu_s^{\varepsilon_q} )(\dd y)\dd s \label{proof:eq:exponentail-completness-1}\\
        &+ \dfrac12\int_0^t\int_{\RRP}\int_{\RRP}K_\alpha(x,y)f(x+y) (G^{\varepsilon_p}_s - G^{\varepsilon_q}_s)(x,y)\nu_s^{\varepsilon_q}(\dd x)\nu_s^{\varepsilon_q}(\dd y) \dd s.\label{proof:eq:exponentail-completness-2}
    \end{align}
    \label{proof:eq:exponentail-completness}
    \end{subequations}
    Let us start by bounding \eqref{proof:eq:exponentail-completness-1}, we have for all continuous $|f| \leq \psi$:
    \begin{multline*}
    ...
    \end{align*}
    We get for \eqref{proof:eq:exponentail-completness-1}:
    \begin{multline*}
        \int_0^t \int_{\RRP}\int_{\RRP}K_\alpha(x,y)f(x+y) G^{\varepsilon_p}_s(x,y)(\nu_s^{\varepsilon_p} + \nu_s^{\varepsilon_q})(\dd x)(\nu_s^{\varepsilon_p} -\nu_s^{\varepsilon_q} )(\dd y)\dd s \\
        \leq 2\| \mu\|_{\psi + \psi_1}e^{T\|\mu\|_{psi_1}} \int_0^t \left\|\nu_s^{\varepsilon_p} - \nu_s^{\varepsilon_q} \right\|_{\psi + \psi_1} \dd s.
    \end{multline*}
    For \eqref{proof:eq:exponentail-completness-2}, we just have to bound:
    \begin{align*}
    ...
    \end{align*}
    By Lemma \ref{lem:Lip-smol-TV} there exists a constant $C(T,\mu)$ such that:
    \begin{align*}
    ...
    \end{align*}
    The bound of \eqref{proof:eq:exponentail-completness-2} reads:
    \begin{multline*}
    ...
    \end{align*}
    We can now use once more the Lemma \ref{lem:exponential-moments-epsilon} to get the complete bound \eqref{proof:eq:exponentail-completness-2}:
    \begin{align*}
        \int_0^t\int_{\RRP}\int_{\RRP}K_\alpha(x,y)f(x+y) (G^{\varepsilon_p}_s - G^{\varepsilon_q}_s)(x,y)\nu_s^{\varepsilon_q}(\dd x)\nu_s^{\varepsilon_q}(\dd y) \dd s \\
        \leq C\left(T,\|\mu\|_{\psi_2}\right)\left(\varepsilon_p \vee \varepsilon_q \right)^{\alpha} \left(\|\mu\|_{\psi + \psi_2}\right)^2.
    \end{align*}
    Finally we can put \eqref{proof:eq:exponentail-completness} in Gronwall's shape for all continuous functions $|f| \leq \psi$ we have:
    \begin{multline*}
    ...
    \end{multline*}
    Notice that the function $\psi + \psi_1$ is also non increasing, consequently taking $|f| \leq \psi + \psi_1$ will only change the previous inequality up to a constant. By taking the suppremum over continuous functions dominated by $\psi + \psi_1$ one can deduce:
    \begin{multline*}
    ...
    \begin{align*}
        \left\| \nu_t - h_t\mu_t \right\|_{\psi_1} \leq \left\|\nu_t - \nu^\varepsilon_t\right\|_{\psi_1}  + \left\|h^\varepsilon_t\mu^\varepsilon_t - h_t\mu^\varepsilon_t\right\|_{\psi_1} + \left\|h_t\mu^\varepsilon_t - h_t\mu_t\right\|_{\psi_1}.
    \end{align*}
    We already showed that the first term vanishes. For the second we have:
    \begin{align*}
        \left\|h^\varepsilon_t\mu^\varepsilon_t - h_t\mu^\varepsilon_t\right\|_{\psi_1} 
        &= \left\|(h^\varepsilon_t - h_t)\mu^\varepsilon_t\right\|_{\psi_1}.
    \end{align*}
    A simple computations yields for all $x \in \RRP$:
    \begin{align*}
        \left| h^\varepsilon_t(x) - h_t(x) \right| \leq (1 + \psi_1(x))\int_0^t \| \mu^\varepsilon_s - \mu_s \|_{\psi_1} \dd s.
    \end{align*}
    and therefore:
    \begin{align*}
        \left\|h^\varepsilon_t\mu^\varepsilon_t - h_t\mu^\varepsilon_t\right\|_{\psi_1} &\leq \|\mu_t\|_{\psi_2} \int_0^t \| \mu^\varepsilon_s - \mu_s \|_{\psi_1} \dd s \\
        &\leq  \|\mu\|_{\psi_2} C(T,\|\mu\|_{\psi_2}) \| \mu^\varepsilon - \mu \|_{\psi_1} \xrightarrow[\varepsilon \to 0]{} 0,
    \end{align*}
    where we used \ref{lem:Lip-smol-TV}. For the third and final term we simply have since $0 \leq h_t \leq 1$:
    \begin{align*}
        \left\|h_t\mu^\varepsilon_t - h_t\mu_t\right\|_{\psi_1} \leq \left\|\mu^\varepsilon_t - \mu_t\right\|_{\psi_1}
    \end{align*}
    and once again vanishes thanks to Lemma \ref{lem:Lip-smol-TV}. Thus concluding this proof.
\end{proof}

\subsection{Wasserstein bounds}

We denote by $t \mapsto \varphi_t(\mu)$ the solution to the $\SCE$ of kernel $K_\alpha$ started from $\mu$ where $\mu \in \MC_{\psi_3}^+(\RRP)$ and by $t \mapsto \delta_\mu\varphi_t(\mu;z)$ its derivative. We define the Lip pseudo norm as:
\begin{align*}
    Lip(f) = \sup\left\lbrace \dfrac{|f(x) - f(y)|}{d_\alpha(x,y)} , (x ,y) \in \left(\RR ^+_*\right)^2,x\neq y \right\rbrace 
\end{align*}
Recall the definition of our distance for $\mu,\nu \in \MC_{\psi_1}^+(\RRP)$:
\begin{align*}
    W_{\psi_1}(\mu,\nu) = \sup\limits_{Lip(f) \leq 1, |f| \leq 1 + \psi_1} \brac{f,\mu - \nu} + \left|\vphantom{A^2} \brac{1,\mu - \nu} \right|.
\end{align*}
...
This technical lemma contains computations that will be repeteaded in most of the results of this section.
\begin{lemma}\label{lem:technical_G_h}
    Let $\Proc{\mu_t}, \Proc{\tilde{\mu}_t}$ be two solutions of the Smoluchowski coagulation equation of kernel $K_\alpha$ of initial value respectively $\mu,\tilde{\mu}$. We introduce:
    \begin{align*}
        h_t(x) = \exp\left(\int_0^t \int_{\RRP} K_\alpha(x,y) \mu_s(\dd y) \dd s \right) 
    \end{align*}
    and $\tilde{h}_t$ similarly. We also introduce the functions:
    \begin{align*}
    ...
    \end{align*}
    and $\tilde{G}_tf$ similarly. The following assertions are true.
    \begin{enumerate}[label=(\roman*)]
        ...
        \item For all $x \in \RRP$,
        \[
            \left|\partial_x \left( h_t(x)\right)^{-1}- \partial_x\left( \tilde{h}_t(x)\right)^{-1} \right|\leq \alpha \left(1 + t\|\mu\| \right)x^{-1-\alpha} \left(1 + t\|\mu\| \right) \int_0^t W_{\psi_1}(\mu_s,\tilde{\mu}_s) \dd s.
        \]
        \item For all $x,y \in \RRP$, 
        \[|G_t(x,y)| \leq (1 + x^{-\alpha})(1 + y^{-\alpha})e^{-\int_0^t \brac{\psi_1,\mu_s}\dd s}.
        \]
        \item For all $x,y \in \RRP$, 
        \[\left|\partial_x G_t(x,y) \right| \leq C \alpha  x^{-1-\alpha} e^{-\int_0^t \brac{\psi_1,\mu_s}\dd s}.
        \]
        \item For all $x,y \in \RRP$,
        \[\left|G_t(x,y)- \tilde{G}_t(x,y)\right| \leq C(1 + x^{-2\alpha})(1 + y^{-2\alpha})\int_0^t W_{\psi_1}(\mu_s,\tilde{\mu}_s) \dd s .
        \]
        
        \item For all $x,y \in \RRP$, 
        \[\left|\partial_x \left(G_t(x,y)- \tilde{G}_t(x,y)\right)  \right| \leq C\alpha x^{-1-\alpha}(1 + x^{-\alpha})(1 + y^{-2\alpha})\int_0^t W_{\psi_1}(\mu_s,\tilde{\mu}_s) \dd s.
        \]
    \end{enumerate}
\end{lemma}
\begin{proof}
    ...
    \end{align*}
    We can bound the second term using $(ii)$, as for the first term we have:
    \begin{align*}
    ...
    \begin{align*}
        |g(z)| \leq z^{-\alpha} + 2x^{-\alpha} + y^{-\alpha} \leq C(1 + z^{-\alpha})(1 + x^{-\alpha})(1 + y^{-\alpha})
    \end{align*}
    Therefore:
    \begin{multline*}
        ...
        \leq K_\alpha(x,y)(1 + x^{-\alpha})(1 + y^{-\alpha}) \int_0^t W_{\psi_1}(\mu_s,\tilde{\mu}_s) \dd s\\
        \leq (1 + x^{-2\alpha})(1 + y^{-2\alpha}) \int_0^t W_{\psi_1}(\mu_s,\tilde{\mu}_s) \dd s
    \end{multline*}
    
    $(vii)$
\end{proof}

\subsection{Control of rescaled solutions} 
For the last result of this part we are going to need the following technical lemma. It gives some regularity properties of the $\SCE$ solutions with the Wasserstein distance associated with the distance $d_{p\alpha}$. Aforementioned Wasserstein distance is defined by:
\begin{align*}
    W_{1,\psi_p}(\mu,\nu) = \sup_{\substack{Lip_{d_{p\alpha}}(f) \leq 1 \\ |f| \leq 1 + \psi_p}} \brac{f,\mu -\nu},
\end{align*}
where for a distance $d$ on $\RRP$,
\begin{align*}
    Lip_d(f) = \sup\left\lbrace \dfrac{\left|f(x) - f(y) \right|}{d(x,y)}, (x,y) \in (\RRP)^2, x \neq y \right\rbrace .
\end{align*}

We give a lemma controlling the derivative of the $\SCE$,
\begin{lemma}\label{lem:wass-rescaled-derivative}
    Let $z_1,z_2 \in \RRP$, the following assertions hold:
    \begin{itemize}
        \item Assume that $\mu \in \MC_{\psi_3}(\RRP)$ for all $f \in Lip_1(d_\alpha)$:
            \begin{align*}
            ...
            \end{align*}
        \item For $p \geq 1$ let $p' = \max(p+1,3)$, assume that $\mu \in \MC_{\psi_{p'}}(\RRP)$ for all $f \in Lip_1(d_{p\alpha})$:
            \begin{align*}
...
\end{lemma}
\begin{proof}
    We introduce the signed measure:
    \begin{align*}
        m_t = \delta_\mu \varphi_t(\mu;z_1) - \delta_\mu \varphi_t(\mu;z_2),
    \end{align*}
    and by slight abuse of notation denote $W_{\psi}(m_t) = W_{\psi}(\delta_\mu \varphi_t(\mu;z_1), \delta_\mu \varphi_t(\mu;z_2))$. We denote by $\Proc{\mu_t} = \Proc{\varphi_t(\mu)}$, We have for for all $t\geq 0$, $f \in C_b(\RRP)$:
    \begin{multline*}
        \brac{f,h_tm_t} = f(z_1) - f(z_2) + \int_0^t \brac{f \partial_s h_s,m_s} \dd s \\
        + \int_0^t \int_{\RRP}\int_{\RRP}K_\alpha(x,y) \left[fh_t(x+y) - fh_t(x) - fh_t(y) \right] \mu_s(\dd x) m_s(\dd y) \dd s \\
        = f(z_1) - f(z_2) + \int_0^t \int_{\RRP}\int_{\RRP}K_\alpha(x,y) \left[fh_t(x+y) - fh_t(x)\right] \mu_s(\dd x) m_s(\dd y) \dd s
    \end{multline*}
    ...
    \begin{subequations}
    \begin{align}
        \brac{f,h_tm_t} = f(z_1) - f(z_2) &+ \int_0^t \brac{H_sf,h_s\mu_s \otimes h_s m_s} \dd s\label{proof:eq:wass-rescaled-SCE-1}\\
    ...
    \end{align*}    
    Notice that we would have to put $H_tf(y,x)$ to be really in accord with our computations so far. However $H_tf$ is symmetric so this hardly matters. This remark will be import to handle \label{proof:eq:wass-rescaled-SCE-2} as $I_tf$ is not symetric. 
    ...
    \end{align*}
    is Lipschitz and estimate its consant. First we have:
    \begin{align*}
        \left|B_tf(x) \right| 
    ...
    Finally:
    \begin{align*}
        \brac{f,h_tm_t} \leq d_{p\alpha}(z_1,z_2) + C(1 + T\|\mu\|)\|\mu\|_{\psi_{p+1}}\int_0^t  W_{\psi_1}(h_sm_s)\dd s,
    \end{align*}
    and by letting the LHS go to the supremum:
    \begin{align*}
        W_{\psi_p}(h_tm_t) \leq d_{p\alpha}(z_1,z_2) + C(1 + T\|\mu\|)\|\mu\|_{\psi_{p+1}}\int_0^t  W_{\psi_1}(h_sm_s)\dd s.
    \end{align*}
    If $p = 1$ then a Gronwall's lemma gives us the first result. Then for $p > 1$ we can use the $\psi_1$ bound and proceed by brute force.
\end{proof}
...
We state and prove the main result of this part.
\begin{proof}
    ...
    \end{align*}
    Step $1$: We show that $W_L(m_t - \tilde{m}_t) \leq W_L(h_tm_t - \tilde{h}_t\tilde{m}_t) + C(T,\mu,\tilde{\mu})d_\alpha(z_1,z_2)W_{\psi_1}(\mu,\tilde{\mu})$.

    Let $f$ be such that $Lip(f) \leq 1$ and $|f| \leq 1$ assume that at time $t$: \red{Attention supoposer ça n'est pas possible pour tout x}
    \begin{align}\label{proof:eq:wass-huge-assumption1}
        \int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s \geq 0,
    \end{align}
    In the opposite case we can simply swap the roles of $\mu$ and $\tilde{\mu}$ in what follows.
    \begin{align}\label{eq:proof:wass-finale-step-1}
    ...
    \end{align}
    Let us start with the first term, we already saw in the proof of Proposition \ref{prop:wass-derivative} that the function $f(h_t)^{-1}$ is Lipschitz with a constant below $1 + \int_0^t \brac{1,\mu_s} \dd s \leq 1 + T\|\mu\|$. It comes that:
    \begin{align*}
        \brac{f(h_t)^{-1},h_tm_t - \tilde{h}_t\tilde{m}_t}
    ...
    \end{align*}
    and show that it is Lpischitz in order to use the Proposition \ref{prop:wass-derivative} and get the desired bound. We differentiate $g$:
    \begin{multline*}
        g'(x) = f'(x) \left(e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s} - 1\right) e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s}\\
            - f(x) \int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\times  e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) \mu_s(\dd x)\dd s} \\
            - f(x) \left(e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s} - 1\right)\\
            \times  \int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) \tilde{\mu}_s(\dd y)\dd s \times e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s}.
    \end{multline*}
    We now use the assumption \eqref{proof:eq:wass-huge-assumption1}, we have:
    \begin{multline*}
        |g'(x)| \leq |f'(x)|\left|\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\right|e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s}\\
        + |f(x)| \left|\int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\right|e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s}\\
        + |f(x)| \left|\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\right|\\
        \times  \int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) \tilde{\mu}_s(\dd y)\dd s \times e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s}.
    \end{multline*}
    Bounding the first term:
    \begin{multline*}
        |f'(x)|\left|\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\right|e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s} \\
        \leq \alpha x^{-1-\alpha} (1 + x^{-\alpha })e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s}\int_0^t  W_{\psi_1}(\mu_s,\tilde{\mu}_s)\dd s\\
        \leq \alpha x^{-1-\alpha} \left(1 + \dfrac{1}{\int_0^t \brac{1,\tilde{\mu}_s}\dd s}\right)\int_0^t  W_{\psi_1}(\mu_s,\tilde{\mu}_s)\dd s 
    \end{multline*}
    We know that $t\mapsto \brac{1,\tilde{\mu}_t}$ is non increasing and therefore:
    \begin{align*}
         \dfrac{1}{\int_0^t \brac{1,\tilde{\mu}_s}\dd s} \leq \dfrac{1}{t \brac{1,\tilde{\mu}_t}}.
    \end{align*}
    Now applying \ref{prop:wass-sce} gives us the bound:
    \begin{multline*}
       |f'(x)|\left|\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\right|e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s} \\
        \leq \alpha x^{-1-\alpha} C(T,\mu,\tilde{\mu})W_{\psi_1}(\mu,\tilde{\mu}) \left(t + \dfrac{1}{\brac{1,\tilde{\mu}_t}}\right).
    \end{multline*}
    Finally the lower bound on $\brac{1,\tilde{\mu}_t}$ found in \red{Ajouter le bound} yields for $t \in [0,T]$:
    \begin{multline*}
        |f'(x)|\left|\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\right|e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s} \\
        \leq \alpha x^{-1-\alpha} C(T,\mu,\tilde{\mu})W_{\psi_1}(\mu,\tilde{\mu}) \left(t + C(\|\tilde{\mu}\|_{\psi_1},\brac{1,\tilde{\mu}}) t^{\frac{1-2^{-\alpha}}{2-2^{-\alpha}}}\right) \\
        \leq \alpha x^{-1-\alpha} C(T,\mu,\tilde{\mu})W_{\psi_1}(\mu,\tilde{\mu}).
    \end{multline*}
    For the second term we have:
    \begin{multline*}
        |f(x)| \left|\int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\right|e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s} \\
        \leq \alpha x^{-1-\alpha} \int_0^t \left|\vphantom{A^2}\brac{1,\mu_s-\tilde{\mu}_s} \right|\dd s \times e^{-\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s}\\
        \leq \alpha x^{-1-\alpha} \int_0^t W_{\psi_1}\left(\mu_s, \tilde{\mu}_s \right) \dd s\\
        \leq \alpha x^{-1-\alpha} C(T,\mu,\tilde{\mu})W_{\psi_1}(\mu,\tilde{\mu}).
    \end{multline*}
    For the third and last term \red{RAPPEL: c'est ici que l'hypothèse bounded et pas $f \leq 1 + x^{-\alpha}$ est importante}:
    \begin{align*}
        \int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s &\leq (1  + x^{-\alpha}) \int_0^t W_{\psi_1}\left(\mu_s, \tilde{\mu}_s \right) \dd s \\
        \int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) \tilde{\mu}_s(\dd y)\dd s &\leq \alpha x^{-1-\alpha} T\brac{1,\mu},
    \end{align*}
    and therefore
    \begin{multline*}
        |f(x)| \left|\int_0^t \int_{\RRP} K_\alpha(x,y) (\mu_s-\tilde{\mu}_s)(\dd y)\dd s\right|\int_0^t \int_{\RRP} \partial_x K_\alpha(x,y) \tilde{\mu}_s(\dd y)\dd s \times e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s}\\
        \leq \alpha x^{-1-\alpha} T\brac{1,\tilde{\mu}}(1  + x^{-\alpha})e^{-\int_0^t \int_{\RRP} K_\alpha(x,y)\tilde{\mu}_s(\dd y) \dd s} \int_0^t W_{\psi_1}\left(\mu_s, \tilde{\mu}_s \right) \dd s \\
        \leq \alpha x^{-1-\alpha} C(T,\mu,\tilde{\mu})W_{\psi_1}(\mu,\tilde{\mu}).
    \end{multline*}
    Finally we found a constant $C(T,\mu,\tilde{\mu})$ such that:
    \[
    |g'(x)| \leq \alpha x^{-1-\alpha} C(T,\mu,\tilde{\mu}) W_{\psi_1}(\mu,\tilde{\mu}).
    \]
    Therefore by Proposition \ref{prop:wass-derivative} the second term in the RHS of \eqref{eq:proof:wass-finale-step-1} enjoys the bound:
    \begin{align*}
        \brac{g,\tilde{h}_t\tilde{m}_t}=\brac{g,\tilde{h}_t\left(\vphantom{A^2}\delta_\mu \varphi_t(\tilde{\mu}:z_1) - \delta_\mu \varphi_t(\tilde{\mu}:z_2)\right)} \leq d_\alpha(z_1,z_2)C(T,\mu,\tilde{\mu}) W_{\psi_1}(\mu,\tilde{\mu}).
    \end{align*}
    It finally comes that the complete expression of \eqref{eq:proof:wass-finale-step-1} is bounded as so:
    \begin{align*}
        \brac{f,m_t - \tilde{m}_t} \leq (1 + T\|\mu\|) W_L(h_tm_t - \tilde{h}_t\tilde{m}_t) + C(T,\mu,\tilde{\mu}) d_\alpha(z_1,z_2) W_{\psi_1}(\mu,\tilde{\mu}).
    \end{align*}
    This completes the first step of the proof by letting the term on the LHS got to the supremum value.

    Step $2$: Show that for all $t \in [0,T] W_{\psi_1}(h_tm_t - \tilde{h}_t\tilde{m}_t) \leq C(T,\mu,\tilde{\mu}) d_\alpha(z_1,z_2) W_{\psi_1}(\mu,\tilde{\mu})$. We start with some notations, we have for all $f \in C_b(\RR^+)$:
    \begin{align*}
        \dfrac{\dd}{\dd t} \brac{f,h_t m_t} = \brac{G_tf,h_t\mu_t \otimes h_tm_t}
    \end{align*}
    with
    \begin{align*}
        G_tf(x,y) =  \left(h_t(x)\right)^{-1}K_\alpha(x,y)\left[fh_t(x+y)h_t(y) - f(y)\right].
    \end{align*}
    We also introduce:
    \begin{align*}
        \tilde{G}_tf(x) =  \left(\tilde{h}_t(x)\tilde{h}_t(y)\right)^{-1}K_\alpha(x,y)\left[f\tilde{h}_t(x+y) - f(y)\right].
    \end{align*}
    One gets for all Lipschitz functions $f$ such that $Lip(f) \leq 1$ and $|f| \leq 1$ that:

    \begin{subequations}
    \begin{align}
        \brac{f, h_tm_t - \tilde{h}_t\tilde{m}_t} =& \brac{G_tf,h_t\mu_t \otimes \left(h_tm_t - \tilde{h}_t\tilde{m}_t \right) }\label{proof:eq:wass-finale-step-2-1}\\ 
        &+ \brac{G_tf,\left(h_t\mu_t - \tilde{h}_t\tilde{\mu}_t\right)\otimes \tilde{h}_t\tilde{m}_t}\label{proof:eq:wass-finale-step-2-2} \\ 
        &+ \brac{G_tf - \tilde{G}_tf,\tilde{h}_t\tilde{\mu}_t\otimes \tilde{h}_t\tilde{m}_t}\label{proof:eq:wass-finale-step-2-3} 
    \end{align}
    \label{proof:eq:wass-finale-step-2}
    \end{subequations}
    We deal first with \eqref{proof:eq:wass-finale-step-2-1} we have:
    \begin{align*}
        \brac{G_tf,h_t \mu_t \otimes \left(h_tm_t - \tilde{h}_t\tilde{m}_t \right) } = \brac{A_tf,h_tm_t - \tilde{h}_t\tilde{m}_t}
    \end{align*}
    where
    \begin{align*}
        A_tf(x) =  \left(h_t(x)\right)^{-1}\int_{\RRP} K_\alpha(x,y)\left[fh_t(x+y) - fh_t(y)\right]\mu_t(\dd y).
    \end{align*}
    We already know from the proof of Proposition \ref{prop:wass-derivative} that $A_tf$ is Lpischitz with constant below $\left(1 + T\|\mu\|\right)\|\mu\|_{\psi_2}$ and therefore:
    \begin{align*}
        \brac{G_tf,h_t\mu_t \otimes \left(h_tm_t - \tilde{h}_t\tilde{m}_t \right) } &\leq \left(1 + T\|\mu\|\right)\|\mu\|_{\psi_2} W_{\psi_1}\left(h_tm_t -\tilde{h}_t\tilde{m}_t \right)
    \end{align*}
    For the term \eqref{proof:eq:wass-finale-step-2-2} we can use Proposition \ref{prop:characterisation-lip-1} :
    \begin{multline*}
        \partial_x G_tf(x,y) = -\int_0^t \int_{\RRP}\partial_x K_\alpha(x,y) \mu_t(\dd y)G_tf(x,y) + \partial_x K_\alpha(x,y)\left(h_t(x)h_t(y)\right)^{-1}\left[fh_t(x+y) - f(y)\right]\\
        + 
    \end{multline*}


    The rest of this step is devoted to bounding the remaining term. We proceed similarly as in the first step by showing that $A_tf - \tilde{A}_tf$ is Lpischitz with a constant controlled by $W_{\psi_1}(\mu,\tilde{\mu})$ and use Proposition \ref{prop:wass-derivative} to conclude. We differentiate the latter function:
    \begin{align*}
        A_tf'(x) - \tilde{A}_tf'(x) = B_tf(x) + C_tf(x) + D_tf(x)
    \end{align*}
    \begin{align*}
        B_tf(x) =& -\int_0^t \int_{\RRP}\partial_x K_\alpha(x,y) \mu_t(\dd y) A_tf(x) + \int_0^t \int_{\RRP}\partial_x K_\alpha(x,y) \tilde{\mu}_t(\dd y)\tilde{A}_tf(x) \\
        C_tf(x) =& \left(h_t(x)\right)^{-1}\int_{\RRP}\partial_x K_\alpha(x,y)\left[fh_t(x+y)- fh_t(y)\right]\mu_t(\dd y)\\
         & - \left(\tilde{h}_t(x)\right)^{-1}\int_{\RRP}\partial_x K_\alpha(x,y)\left[f\tilde{h}_t(x+y) - f\tilde{h}_t(y)\right]\tilde{\mu}_t(\dd y) \\
        D_tf(x) =& \left(h_t(x)\right)^{-1}\int_{\RRP}K_\alpha(x,y) h_t(x+y)\left[f'(x+y) -\int_0^t \int_{\RRP}\partial_x K_\alpha(x,y) \mu_t(\dd y) f(x+y) \right]\mu_t(\dd y) \\
        &- \left(\tilde{h}_t(x)\right)^{-1}\int_{\RRP}K_\alpha(x,y) \tilde{h}_t(x+y)\left[f'(x+y) -\int_0^t \int_{\RRP}\partial_x K_\alpha(x,y) \mu_t(\dd y) f(x+y) \right]\tilde{\mu}_t(\dd y)
    \end{align*}
